    ...
    <div class="container mx-auto px-6 flex justify-between items-center gap-4">
        ...
        <div class="flex items-center gap-3 md:gap-4">
            ...
            <div class="hidden sm:flex items-center gap-2 mr-2">
                <a href="/language/it"
                    class="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all {{ app()->getLocale() == 'it' ? 'bg-vendly text-white shadow-lg shadow-vendly/20' : 'bg-white text-[#08B2B4] border border-vendly/30 hover:bg-vendly/5' }}">
                    <span
                        class="w-2.5 h-2.5 rounded-full {{ app()->getLocale() == 'it' ? 'bg-white' : 'border border-vendly/40 bg-white' }}"></span>
                    IT
                </a>
                <a href="/language/en"
                    class="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all {{ app()->getLocale() == 'en' ? 'bg-vendly text-white shadow-lg shadow-vendly/20' : 'bg-white text-[#08B2B4] border border-vendly/30 hover:bg-vendly/5' }}">
                    <span
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100"
                            ...
                            <form method="POST" action="{{ route('Auth.logout') }}">
                                @csrf
                                <button type="submit"
                                    class="flex items-center gap-3 w-full text-left px-5 py-3 text-sm text-red-600 hover:bg-red-50 transition-colors font-bold">
            ...
            <!-- Sell Button -->
            <a href="{{ route('Frontoffice.vendere') }}"
                class="hidden md:flex items-center justify-center bg-vendly text-white font-bold rounded-xl px-6 py-2.5 text-sm transition-all hover:bg-[#079fa1] hover:shadow-lg hover:shadow-vendly/20 active:scale-95 leading-none shrink-0">
...
<div 
    ...
    class="lg:hidden bg-white px-6 py-4 border-b border-gray-100"
    style="display: none;"
>
    <form action="{{ route('Frontoffice.ricerca') }}" method="GET" class="w-full">
        <div class="relative flex items-center group">
            <span class="material-symbols-outlined absolute left-3 text-gray-400 text-lg transition-colors group-focus-within:text-vendly select-none">search</span>
            <input type="text" name="query" placeholder="{{ __('message.search_products') }}"
