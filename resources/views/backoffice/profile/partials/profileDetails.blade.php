            ...
            <div class="px-5 pb-5 md:px-10 md:pb-10 lg:px-12 lg:pb-12">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-10">
                    ...
                    <div>
                        <span
                            class="text-xs text-gray-400 uppercase font-bold tracking-wider flex items-center gap-2 mb-1">
                            <span class="material-symbols-outlined text-sm">call</span> {{ __('message.phone') }}
                        </span>
                        <p class="text-base md:text-lg text-gray-700 font-medium">
    ...
    <div x-show="openModal" class="fixed inset-0 z-50 flex items-center md:items-center justify-center md:p-6" x-cloak
        ...
        <div class="relative bg-white md:rounded-2xl rounded-t-2xl border border-gray-100 shadow-2xl w-full md:max-w-2xl max-h-[92vh] overflow-y-auto p-5 md:p-10"
            ...
            <div class="text-center mb-5 md:mb-8">
                <h1 class="text-xl md:text-3xl font-bold text-gray-900">{{ __('message.edit_user') }}</h1>
                <p class="text-gray-500 text-xs md:text-sm mt-1">{{ __('message.update_your_info') }}</p>
            </div>
            ...
            <form action="{{ route('Auth.updateUser', auth()->user()) }}" method="POST">
                ...
                <div class="grid grid-cols-2 gap-3 md:gap-4">
                    <div class="flex flex-col">
                        <label
                            class="text-[10px] font-bold uppercase text-gray-400 mb-1 ml-1">{{ __('message.name') }}</label>
                    ...
                    <div class="flex flex-col">
                        <label
                            class="text-[10px] font-bold uppercase text-gray-400 mb-1 ml-1">{{ __('message.surname') }}</label>
                    ...
                    <div class="flex flex-col">
                        <label
                            class="text-[10px] font-bold uppercase text-gray-400 mb-1 ml-1">{{ __('message.phone') }}</label>
                    ...
                    <div class="flex flex-col">
                        <label
                            class="text-[10px] font-bold uppercase text-gray-400 mb-1 ml-1">{{ __('message.date_of_birth') }}</label>
                    ...
                    <div class="flex flex-col">
                        <label
                            class="text-[10px] font-bold uppercase text-gray-400 mb-1 ml-1">{{ __('message.street') }}</label>
                    ...
                    <div class="flex flex-col">
                        <label
                            class="text-[10px] font-bold uppercase text-gray-400 mb-1 ml-1">{{ __('message.city') }}</label>
                    ...
                    <div class="flex flex-col">
                        <label
                            class="text-[10px] font-bold uppercase text-gray-400 mb-1 ml-1">{{ __('message.zip_code') }}</label>
                ...
                <div class="mt-5 md:mt-10 flex flex-col md:flex-row gap-2">
                    <button type="button" @click="openModal = false"
                        class="order-2 md:order-1 flex-1 py-2.5 text-gray-500 font-bold hover:underline text-sm md:text-base">{{ __('message.cancel') }}</button>
                    <button type="submit"
                        class="order-1 md:order-2 flex-[2] bg-gray-900 text-white font-bold py-3 rounded-xl hover:bg-gray-800 transition text-sm md:text-base">{{ __('message.save_changes') }}</button>
                </div>
            </form>
        </div>
    </div>
