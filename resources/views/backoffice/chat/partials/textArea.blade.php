...
<div id="messages-container" class="flex-1 p-4 overflow-y-auto bg-gray-50 flex flex-col">
    ...
    @else
        ...
        <div class="flex-1 flex flex-col items-center justify-center opacity-70">
            ...
            <p class="text-gray-500 text-sm font-medium">{{ __('message.start_conversation') }}</p>
            <p class="text-gray-400 text-xs mt-1">{{ __('message.ask_about_item') }}</p>
        </div>
    @endif
</div>
...
<div class="p-3 border-t border-gray-200 bg-white shrink-0">
    <form id="chat-form" action="{{ route('Backoffice.sendMessage') }}" method="POST" class="flex gap-2 items-end">
        ...
        <textarea id="message-input" name="content" rows="1" placeholder="{{ __('message.write_message') }}" required
        ...
        <button type="submit"
            ...
            <span class="hidden md:inline">{{ __('message.send') }}</span>
